...
export default function VitoPizzaApp() {
  ...
  const currentBannerText = useMemo(() => {
      ...
      pData.filter(p => p.avg > 4.7 && p.count > 2).forEach(p => msgs.push(`${t.hotPick} ${p.displayName}!`));
  ...
  async function modificarPedido(p: any, acc: 'sumar' | 'restar') {
    ...
    if (acc === 'sumar') { if (p.stockRestante <= 0) return; const { error } = await supabase.from('pedidos').insert([{ invitado_nombre: nombreInvitado, pizza_id: p.id, cantidad_porciones: 1, estado: 'pendiente' }]); if (!error) mostrarMensaje(`${t.successOrder} ${p.displayName}!`, 'exito'); } else { const { data } = await supabase.from('pedidos').select('id').eq('pizza_id', p.id).ilike('invitado_nombre', nombreInvitado.trim()).eq('estado', 'pendiente').order('created_at', { ascending: false }).limit(1).single(); if (data) { await supabase.from('pedidos').delete().eq('id', data.id); mostrarMensaje(`${t.successCancel} ${p.displayName}`, 'info'); } }
  }
      ...
      return (
        <div className={`min-h-screen flex items-center justify-center p-4 font-sans ${base.bg}`}>
            <div className={`w-full max-w-md p-8 rounded-3xl border shadow-2xl text-center ${base.card}`}>
                ...
                <div className={`mt-8 pt-4 border-t border-white/5`}><Link href="/admin" className={`text-xs flex items-center justify-center gap-1 ${base.subtext} hover:${base.text}`}><Lock size={12}/> Admin</Link></div>
            </div>
        </div>
      );
  }
  ...
  return (
    <div className={`min-h-screen font-sans pb-28 transition-colors duration-500 overflow-x-hidden ${base.bg}`}>
      ...
      {/* ONBOARDING OVERLAY */}
      {showOnboarding && (
          <div onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEndSwipe} className="fixed inset-0 z-[100] bg-white/95 backdrop-blur-xl flex flex-col items-center justify-center p-8 text-center text-neutral-900 animate-in fade-in duration-500 select-none">
      ...
      <div className={`w-full p-6 pb-6 rounded-b-[40px] bg-gradient-to-br ${currentTheme.gradient} shadow-2xl relative overflow-hidden`}>
         ...
         <div className="relative z-10 pt-16"><div className="mb-6"><h1 className="text-3xl font-bold leading-tight drop-shadow-md text-white">{t.welcomeTitle} <br/> <span className="opacity-80 font-normal text-xl">{t.welcomeSub}</span></h1></div>
      ...
      <div className="px-4 mt-6 relative z-20 max-w-lg mx-auto pb-20">
        <div className={`${base.card} p-2 rounded-2xl border flex items-center gap-3 mb-6`}>
             ...
             <div className="flex-1 pr-2"><label className={`text-[10px] uppercase font-bold ${base.subtext} ml-1`}>{t.whoAreYou}</label>
           ...
           {cargando ? <p className={`text-center ${base.subtext} mt-10 animate-pulse`}>{t.loading}</p> : 
             ...
             pizzasOrdenadas.map(pizza => (
               <div key={pizza.id} className={`${base.card} rounded-[36px] border ${pizza.stockRestante === 0 ? 'border-neutral-200 dark:border-neutral-800' : pizza.cocinando ? 'border-red-600/30' : ''} shadow-lg relative overflow-hidden group ${isCompact ? 'p-3' : 'p-5'}`}>
                   <div className="flex justify-between items-start mb-2"><div className="flex-1"><div className="flex flex-wrap items-center gap-2 mb-1"><h2 className={`font-bold ${isCompact ? 'text-lg' : 'text-2xl'} ${pizza.stockRestante === 0 ? 'text-gray-400 dark:text-neutral-600' : base.text}`}>{pizza.displayName}</h2>
