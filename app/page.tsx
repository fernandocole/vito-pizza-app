...
export default function VitoPizzaApp() {
        ...
        dPiz.forEach(pz => {
             ...
             if (!firstLoadRef.current && flowStep === 'app') { 
                 ...
                 if (estabaCocinando && !pz.cocinando && teniaPendientes) {
                     ...
                     sendNotification(`¡${pz.nombre} Lista!`, `¡Ya está ${estadoListo}! A comer.`, `/?rate=${pz.id}`);
                 }
             } 
  ...
  const enrichedPizzas = useMemo(() => {
      ...
      return pizzas.map(pizza => {
          ...
          return { 
              ...pizza, displayName, displayDesc, stockRestante, target, ocupadasActual: pen % target, 
              faltanParaCompletar: target - (pen % target), avgRating: avg, countRating: countRating, 
              sortRating: sortR, totalPendientes: pen 
          };
      });
  ...
  if (flowStep === 'landing') {
      return (
          <div className={`min-h-screen flex flex-col items-center justify-center p-6 font-sans ${base.bg}`}>
              {/* Botón de Idioma Minimalista (solo texto/bandera) */}
              ...
              <div className="flex-1 flex flex-col items-center justify-center w-full max-w-sm relative z-10">
                  ...
                  {/* Texto Modificado y Traducible (con fallback a local si t no tiene las keys) */}
    ...
    <div className={`min-h-screen font-sans pb-28 transition-colors duration-500 overflow-x-hidden ${base.bg}`}>
      ...
      <div className={`w-full p-6 pb-6 rounded-b-[40px] bg-gradient-to-br ${currentTheme.gradient} shadow-2xl relative overflow-hidden`}>
         ...
         <div className="relative z-10 pt-16">
             <div className="mb-6">
                 {(() => {
                     // RESTAURADO: Usa mensaje de DB si existe, sino usa el default con nombre
                     ...
                     if (msg) {
                         ...
                         // Default si no hay mensaje en DB
                         return (
                             <>
