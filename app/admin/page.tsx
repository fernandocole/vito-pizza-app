...
export default function AdminPage() {
  ...
  const pedidosAgrupados = Array.from(new Set(pedidos.map(p => p.invitado_nombre.toLowerCase()))).map(nombre => {
      ...
      const total = detalle.reduce((acc, d) => acc + (d?.enHorno || 0) + (d?.enEspera || 0), 0);
      return { nombre: nombreReal, detalle, totalPendienteGeneral: total };
  ...
  const toggleB = async (u: any) => { let uid = u.id; if(!uid) { const { data } = await supabase.from('lista_invitados').insert([{ nombre: u.nombre }]).select().single(); uid = data.id; } await supabase.from('lista_invitados').update({ bloqueado: !u.bloqueado }).eq('id', uid); cargarDatos(); };
  const updateM = async (nombre: string, u: any, m: string) => { let uid = u?.id; if(!uid) { const { data } = await supabase.from('lista_invitados').insert([{ nombre: nombre }]).select().single(); uid = data.id; } await supabase.from('lista_invitados').update({ motivo_bloqueo: m }).eq('id', uid); };
  ...
  return (
    <div className={`min-h-screen font-sans pb-24 w-full ${base.bg}`}>
      ...
      <main className="p-4 max-w-4xl mx-auto space-y-4 w-full">
        ...
        {view === 'pedidos' && (<div className="space-y-4"><h2 className={`text-sm font-bold uppercase tracking-widest mb-2 ${base.subtext}`}>Pedidos Activos</h2>{pedidosAgrupados.length === 0 ? <p className={`text-center ${base.subtext}`}>Sin pedidos.</p> : pedidosAgrupados.map((u, i) => { const userDB = invitadosDB.find(x => x.nombre.toLowerCase() === u.nombre.toLowerCase()); return (<div key={i} className={`${base.card} p-4 rounded-2xl border relative`}><button onClick={() => eliminarUsuario(u.nombre, userDB)} className={`absolute top-4 right-4 p-2 rounded-lg ${base.buttonIcon} hover:text-red-500`}><Trash2 size={16} /></button><div className={`flex justify-between border-b pb-2 mb-3 pr-10 ${base.divider}`}><h3 className="font-bold flex items-center gap-2 capitalize text-lg"><User size={18}/> {u.nombre} {u.totalPendienteGeneral > 0 && <span className="bg-red-600 text-white text-[10px] px-2 py-0.5 rounded-full font-bold animate-pulse">ESPERANDO</span>}</h3></div><div className="space-y-2">{u.detalle.map((d: any, k: number) => (<div key={k} className={`flex justify-between items-center text-sm p-2 rounded-lg border ${isDarkMode ? 'bg-black/20 border-white/5' : 'bg-gray-50 border-gray-200'}`}><span>{d.nombre}</span><div className="flex items-center gap-2 text-xs font-bold">{d.enHorno > 0 && (<span className="text-red-500 flex items-center gap-1"><Flame size={12}/> {d.enHorno}</span>)}{d.enEspera > 0 && (<span className="text-yellow-500 flex items-center gap-1"><Clock size={12}/> {d.enEspera}</span>)}{d.entregada > 0 && (<span className="text-green-500 flex items-center gap-1"><CheckCircle size={12}/> {d.entregada}</span>)}<button onClick={(e) => { e.stopPropagation(); eliminarPedidosGusto(u.nombre, d.id); }} className="p-1 ml-2 bg-red-900/20 text-red-500 rounded hover:bg-red-900/40 border border-red-900/30"><XCircle size={14} /></button></div></div>))}</div></div>); })}</div>)}
                ...
                <div className="space-y-2">
                    {allUsersList.map(u => (
                        <div key={u.nombre} className={`p-4 rounded-2xl border flex flex-col gap-2 ${u.bloqueado ? base.blocked : base.card}`}>
                            ...
                            {u.bloqueado && (<input className={`w-full p-2 rounded-lg text-sm outline-none border ${base.input} text-red-400`} placeholder="Motivo..." defaultValue={u.motivo_bloqueo || ''} onBlur={(e) => updateM(u.nombre, u.origen === 'db' ? u : null, e.target.value)} />)}
                        </div>
                    ))}
                </div>
            </div>
      ...
      <nav className={`fixed bottom-0 w-full backdrop-blur-md border-t flex justify-around p-4 pb-8 z-50 ${base.header}`}>
          ...
          <button onClick={() => setView('ranking')} className={`flex flex-col items-center gap-1 ${view === 'ranking' ? currentTheme.text : base.subtext}`}><BarChart3 size={24} /><span className="text-[9px] uppercase font-bold">Rank</span></button>
