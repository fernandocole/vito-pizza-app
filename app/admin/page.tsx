...
import { Pizza, Settings, Plus, Trash2, ChefHat, Eye, EyeOff, CheckCircle, Clock, Flame, LogOut, List, User, Bell, ArrowRight, ArrowDownAZ, ArrowUpNarrowWide, Maximize2, Minimize2, AlertCircle } from 'lucide-react';
...
export default function AdminPage() {
  ...
  // --- LÓGICA MEJORADA DE AGRUPACIÓN DE PEDIDOS ---
  const pedidosAgrupados = Array.from(new Set(pedidos.map(p => p.invitado_nombre.toLowerCase()))).map(nombre => {
      ...
      // Calcular detalles por cada tipo de pizza
      const detalle = pizzas.map(pz => {
          ...
          // Lógica: Si está pendiente Y la pizza se está cocinando, cuenta como "En Horno"
          // Si está pendiente pero la pizza NO se está cocinando, es "En Espera"
          const enHorno = pz.cocinando ? cantPendiente : 0;
          const enEspera = pz.cocinando ? 0 : cantPendiente;
          ...
          return {
              ...
              enHorno: enHorno,
              enEspera: enEspera,
              total: cantEntregada + cantPendiente
          };
      }).filter(Boolean);

      // Calcular totales para ordenar la lista
      ...
      // Ordenar: Primero los que tienen pendientes, luego los que ya están completos
      return b.totalPendienteGeneral - a.totalPendienteGeneral;
  });
    ...
    <div className="min-h-screen bg-neutral-950 flex items-center justify-center p-4 font-sans overflow-hidden">
      <div className="w-full max-w-md bg-neutral-900 p-8 rounded-3xl border border-neutral-800 shadow-2xl">
        ...
        <form onSubmit={ingresar} className="flex flex-col gap-4">
            ...
            <button type="submit" className={`w-full ${currentTheme.color} text-white font-bold py-4 rounded-xl hover:brightness-110 transition`}>ENTRAR</button>
        </form>
            ...
            {metricas.map(p => (
              <div key={p.id} className={`bg-neutral-900 rounded-3xl border relative overflow-hidden transition-all ${p.cocinando ? 'border-red-600/50 shadow-[0_0_30px_rgba(220,38,38,0.1)]' : 'border-neutral-800'} ${isCompact ? 'p-3' : 'p-5'}`}>
                ...
                <div className="flex justify-between items-start mb-2 relative z-10">
                    <div><h3 className={`font-bold flex items-center gap-2 ${isCompact ? 'text-base' : 'text-xl'}`}>{p.nombre}{p.cocinando && <span className="text-[10px] bg-red-600 text-white px-2 py-0.5 rounded-full font-bold animate-pulse">EN HORNO</span>}</h3><p className="text-xs text-neutral-500 flex items-center gap-1 mt-1"><Clock size={12}/> Pendientes: {p.totalPendientes}</p></div>
                    ...
                    <div key={i} className={`bg-neutral-900 p-4 rounded-2xl border ${u.totalPendienteGeneral > 0 ? 'border-neutral-600 shadow-md' : 'border-neutral-800 opacity-80'} relative`}>
                        ...
                        <div className="space-y-2">
                            {u.detalle.map((d: any, k: number) => (
                                <div key={k} className="flex justify-between items-center text-sm bg-black/20 p-2 rounded-lg border border-white/5">
                                    <span className="font-medium text-neutral-300">{d.nombre}</span>
                                    <div className="flex gap-2 text-xs font-bold">
                                        {/* INDICADORES VISUALES */}
                                        {d.enHorno > 0 && (
                                            <span className="bg-red-900/50 text-red-400 border border-red-500/30 px-2 py-1 rounded flex items-center gap-1">
                                                <Flame size={12} /> {d.enHorno}
                                            </span>
                                        )}
                                        {d.enEspera > 0 && (
                                            <span className="bg-yellow-900/30 text-yellow-500 border border-yellow-500/20 px-2 py-1 rounded flex items-center gap-1">
                                                <Clock size={12} /> {d.enEspera}
                                            </span>
                                        )}
                                        {d.entregada > 0 && (
                                            <span className="bg-green-900/30 text-green-500 border border-green-500/20 px-2 py-1 rounded flex items-center gap-1 opacity-60">
